...
min_Y = 27; max_Y = 32.5;  % min and max limit of Y (lat)
...
for i=2:nx-1
    ...
    shading flat;
    [c,g]=contour(yi,zi,cross_inter,[min_Vp:0.8:max_Vp],'k');
    %[c,g]=contour(yi,zi,cross_inter,[5.8:1:8.5],'k','linewidth',0.7);
    ...
    pcolor(yi,zi,cross_inter);
    shading flat;
    [c,g]=contour(yi,zi,cross_inter,[min_Vs:0.46:max_Vs],'k');
    %[c,g]=contour(yi,zi,cross_inter,[5.8:1:8.5],'k','linewidth',0.7);
    clabel(c,g,'fontsize',8);
    for j=1:length(z)
        if abs(y(j))>90
    ...
    print(h,'-dpng',strcat('vp&vs_lon',num2str(X(i)),'_lat',num2str(min_Y),'-',num2str(max_Y),'_Z',num2str(min_Z),'-',num2str(max_Z)),'-r300');
end