%
% plot earhtquake location in lon-dep plane.
%
clear;
...
for i=1:length(z)
    markersize=6.0;
    plot(x(i),z(i),'.','markersize',markersize,'color','k','linewidth',6);
    hold on
end
%axis equal;
axis ij;
axis image;
axis([X(2),X(nx-1),Z(2),Z(nz-4)]);
...
print(h,'-depsc2','lon-dep_reloc','-r300');
