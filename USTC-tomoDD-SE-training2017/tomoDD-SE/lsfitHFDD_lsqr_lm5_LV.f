...
c     Testing...
	...
	enddo
        ...
        open(39,file="LV_curve.dat")

	do damp=damp0/10,damp0,20
...
c       Unweight and rescale G matrix
	   do i=1,nndt
	      rw2(i)        = rw(i)        * wtinv(i) * norm(iw(1+nar+i))
	      rw2(nndt+i)   = rw(nndt+i)   * wtinv(i) * norm(iw(1+nar+nndt+i))
	      rw2(2*nndt+i) = rw(2*nndt+i) * wtinv(i) * norm(iw(1+nar+2*nndt+i))
	      rw2(3*nndt+i) = rw(3*nndt+i) * wtinv(i) * norm(iw(1+nar+3*nndt+i))
	      rw2(4*nndt+i) = rw(4*nndt+i) * wtinv(i) * norm(iw(1+nar+4*nndt+i))
	      rw2(5*nndt+i) = rw(5*nndt+i) * wtinv(i) * norm(iw(1+nar+5*nndt+i))
	      rw2(6*nndt+i) = rw(6*nndt+i) * wtinv(i) * norm(iw(1+nar+6*nndt+i))
	      rw2(7*nndt+i) = rw(7*nndt+i) * wtinv(i) * norm(iw(1+nar+7*nndt+i))
	   enddo
	
c       Compute residuals from d = G*x
	   do i=1,nndt
	      i1=old_data_index(i)
	      d2(i) = -dt_res(i1)*1000.0
	   enddo

	   call aprod(1, m, n, x, d2, leniw, lenrw, iw, rw2)

	   rnorm=0.0
	   do i=1, nndt
	      rnorm=rnorm+d2(i)*d2(i)/1000000
	   enddo

	   rnorm2=0.0
	   do i=1, nndt
	      rnorm2=rnorm2+wt(i)*wt(i)*d2(i)*d2(i)/1000000
	   enddo

	   write(39,*)damp,sqrt(xnorm),sqrt(rnorm),sqrt(rnorm2)
