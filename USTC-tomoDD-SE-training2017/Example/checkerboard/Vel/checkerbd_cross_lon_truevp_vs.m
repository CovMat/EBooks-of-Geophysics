%
% plot X (longitude) cross sections of true checkerboard (vp and vs).
% Hao Guo.
%
clc
clear
...
%%%%%%%%%%%%%%%%%%%%%%%%%% read checkerboard velocity data %%%%%%%%%%%%%%%%%
fid=fopen('../Syn/MOD','r');
gridnumber=fscanf(fid,'%f',[1,4]);
X=fscanf(fid,'%f',[1,nx]);
Y=fscanf(fid,'%f',[1,ny]);
...
for k=1:nz
    for j=1:ny
        for i=1:nx
            vpvs((k-1)*ny+j,i)=fscanf(fid,'%f',1);
        end
    end
end
vs=vp./vpvs;
...
for i=2:nx-1
    ...
    print(h,'-dpng',strcat('inter',num2str(pinter),'_truevp&vs_lon',num2str(X(i)),'_lat',num2str(min_Y),'-',num2str(max_Y),'_Z',num2str(min_Z),'-',num2str(max_Z)),'-r300');
end
